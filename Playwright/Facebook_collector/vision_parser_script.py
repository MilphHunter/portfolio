...
GEONIX_API=os.getenv('GEONIX_API')
...
class VisionParser:
    ...
    async def start_profile(self, folder_id, profile_id):
        ...
        while True:
            ...
            try:
                async with aiohttp.ClientSession(headers=headers) as session:
                    async with session.get(url, headers=headers, json=json_body) as resp:
                        data = await resp.json()
                        if data.get('error', ''):
...
class FacebookParser:
    @staticmethod
    async def get_current_ad_count(page, retries: int = 3, delay: float = 2.0) -> int:
            ...
            async with async_playwright() as p:
                ...
                for i, url in enumerate(urls):
                    ...
                    if url.startswith('chrome-extension://'):
                        if i == 0:
                            start_ad_count = await self.get_current_ad_count(page)
                            if start_ad_count == -1:
                                logger.error('Ошибка при получении начального количества рекламы.')
                                await page.close()
                                return None, None
                            logger.info(f'Начальное количество собранной рекламы: {start_ad_count}')
                        else:
                            end_ad_count = await self.get_current_ad_count(page)
                            if end_ad_count == -1:
                                logger.error('Ошибка при получении конечного количества рекламы.')
                                await page.close()
                                return None, None
                            ad_count = end_ad_count - start_ad_count
